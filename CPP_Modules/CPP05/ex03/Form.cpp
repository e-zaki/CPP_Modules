...
Form::FormNotSignedException::FormNotSignedException(const std::string& msg) : _message(msg) {};
...
void Form::execute(Bureaucrat const & executer) const {
	...
	if (!this->FormSigned())
	{
		throw FormNotSignedException(">>>>> Couldn't execute because Form is not signed.");
	}
		...
		throw GradeTooHighException(">>>>> Grade too high to execute Form.");
	}
}
